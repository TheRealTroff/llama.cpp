...
**Extended an eleventh time 2026-09-06 late (owner: "Ship it!"): + the FA stack `GGML_FA_QT=1 GGML_MM_F16B=1 GGML_FA_GQA_F16=1 GGML_MM_N64_KMAX=20000 GGML_FA_QR=8 GGML_FA_Q16=1`** - branch `gdn-prefill-scan` (which carries `ud-soa-iq4xs`) merged to prod: transposed-Q FA, the GQA f16 decode route, QR (Q^T shared across score tiles + 8 register tiles, gated <= 64K prefill / decode always) and the 16-row query tile (> 32K, prefill only). 8K prefill 63.0 -> 62.2 s, 96K prefill 1254 -> 1015 s (-19%), every sha canonical (`fa-long-context.md`); mint below.
