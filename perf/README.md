...
| `LLAMA_GDN_REPLAY=1` | 0 | recompute-on-rollback for the delta-net snapshots (2026-09-04, branch `gdn-replay-rollback`): 2 state writes per round at any draft depth instead of depth + 1, the rejected tail is re-run from the kept pre-state inside the next round's kernel; byte-identical (f16 pick `95eb7e65977e`, Turbo4 depth 3 `12c3dc6bb2dd`). Measured on the ceiling table's points (prompt 06, 400 tok): 1 slot depth 3/4 only -0.7%/-1.0% round (the replayed tokens are serial recurrence steps and eat half the saving), **4 slots depth 3 -3.3% round / +2.8% aggregate**, 8 slots depth 1 flat; Turbo4 single stream 30.13 vs 30.06 at 600 (over 30 either way on this prod). Also cuts the state cache from 1+depth to 2 groups. Not in the pick yet (owner's call) | gdn-replay-rollback.md |
