#!/bin/bash
# End-to-end width-6 A/B: the SoA scalar w6 kernel against skinny at the depth-5 point.
#
# DFlash n5 and MTP d5 verify six columns. The depth re-sweep says depth 4 cannot reach
...
#   skinny: GGML_MM_SKINNY=6  (width 6 on the simdgroup-matrix kernel, the pick's route)
#   w6:     GGML_MM_SKINNY=7 + GGML_MV_SOA_W6=$W6_ROWS [+ GGML_MV_SOA_W6_HALF=1]
#           (skinny keeps 7..8; width 6 falls to the SoA scalar kernel)
#
# Both arms carry the full 2026-08-28 pick env (W4 v3 + W5 w5r4h + repack). The dflash
# n4 control - the pick point itself - has no width-6 mul_mv and must be inert and
...
TAG=${TAG:-m4-w6-e2e-$(date +%m%d-%H%M)}
NPRED=${NPRED:-600}
REPS=${REPS:-4}
