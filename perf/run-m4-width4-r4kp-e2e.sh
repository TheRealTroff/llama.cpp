#!/bin/bash
# End-to-end width-4 A/B for the r4kp codegen kernels against the landed R2 route.
#
# DFlash depth 3 verifies four columns, the exact width the SoA route selects. All arms keep
# GGML_MV_REPACK=1 and GGML_MV_SOA_W4=1 so allocation, layout and residency are controlled.
# The only arm-level change is the kernel switch:
#
#   r2: one-simdgroup 2x4 SoA dot kernel (landed route)
#   v2: two-simdgroup 4x4 kp2 kernel, signed-int indexing + hoisted planar row pointers
#   v3: v2 with the product in half precision
#
# The DFlash depth-6 control verifies seven columns and must be inert in every arm.
set -euo pipefail
...
TAG=${TAG:-m4-w4-r4kp-e2e-$(date +%m%d-%H%M)}
NPRED=${NPRED:-600}
REPS=${REPS:-4}
...
run_one() {
    ...
    env "${COMMON_ENV[@]}" GGML_MV_SOA_W4_R2="$r2" GGML_MV_SOA_W4_R4KP="$r4kp" \
...
if [ "$PHASES" = all ] || [ "$PHASES" = n3 ]; then
    ...
    run_one warmup-discard warmup r2 3
...
if [ "$PHASES" = all ] || [ "$PHASES" = n6 ]; then
    ...
    for rep in $(seq 1 "$CONTROL_REPS"); do
        if [ $((rep % 2)) -eq 1 ]; then order=(r2 v3); else order=(v3 r2); fi
        for arm in "${order[@]}"; do
            run_one "n6-$arm-r$rep" n6 "$arm" 6
        done
    done
fi
...
for point in ("n3", "n6"):
    ...
    for arm in ("v2", "v3"):
        if "r2" in means and arm in means:
            delta = 100.0 * (means[arm] / means["r2"] - 1.0)
            print(f"  {point} {arm} delta: {delta:+.2f}%")
