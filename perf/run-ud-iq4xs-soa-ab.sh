...
N=${N:-4}
TYPE=${TYPE:-iq4_xs}          # iq4_xs | q4_K | q5_K
ENVNAME=${ENVNAME:-GGML_MV_SOA_IQ4XS}   # GGML_MV_SOA_KQ for the K-quants
mkdir -p "$OUT"
exec > >(tee "$OUT/$TAG.log") 2>&1
echo "=== $TYPE SoA w$N A/B ($ENVNAME): $TAG ==="
...
one() {  # m k variant -> "us kernel"
    ...
    out=$(env GGML_MV_REPACK=2 $ENVNAME=$v "$BIN" perf -o MUL_MAT -b MTL0 \
          -p "type_a=$TYPE,type_b=f32,m=$m,n=$N,k=$k," 2>&1)
    local us=$(echo "$out" | grep -oE '[0-9]+\.[0-9]+ us/run' | head -1 | cut -d' ' -f1)
    local kn=$(echo "$out" | grep -oE 'loaded kernel_mul_mv_[A-Za-z0-9_]+' | grep -v cpy | tail -1 | sed 's/loaded //')
    echo "$us $kn"
}
