...
- **Packager bug, worked around (2026-09-02):** `applegpu-nt` can fail with `cannot find private metadata at offset N` for a specialized function depending on its position in the metallib's function list, not on the kernel. Translation itself succeeds: run with `-stop-after translate` (no `-o`; the native Mach-O lands in the cwd as `<script>.compute-pipeline-0`) and use that file; its `__TEXT` bytes are identical to the packaged output where both exist. `agx-spill-probe.py` does this automatically and reports `via=stage`. Details in the `metal-kernel-prescreen` skill. The findings this skill summarizes are in `perf/air-layout-control.md`.
