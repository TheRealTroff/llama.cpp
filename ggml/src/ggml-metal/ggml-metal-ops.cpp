...
// Pin XL tensors to SoA because the lm_head runs at widths 1-3 before the first w5 verify.
// GGML_MV_SOA_PIN extends that policy to every eligible row count for adaptive widths: a
// first-call di repack would otherwise lock the later SoA routes out for the process lifetime.
// A consumer whose kernel reads the other layout falls back to the plain weights.
static bool ggml_metal_op_mul_mat_try_repack_q4_0(ggml_metal_op_t ctx, const ggml_tensor * op, ggml_metal_buffer_id & bid_src0, uint64_t & nb01_eff) {
    ...
    const bool soa_width = (env_soa_w3 && op->src[1]->ne[1] == 3) || (env_soa_w4 && op->src[1]->ne[1] == 4) || (env_soa_w5 && op->src[1]->ne[1] == 5) ||
    ...
    const bool any_soa = env_soa_w3 || env_soa_w4 || env_soa_w5 || env_soa_w6 || env_soa_w7;
    ...
    // SoA is pinned, but this consumer reads di: use the plain weights.
...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        const bool use_soa_w3 = env_soa_w3 && ne11 == 3 && ne12 == 1 && ne13 == 1 && use_f16y && use_di &&
                                ...
                                ggml_metal_mul_mat_soa_w4_rows(ne01);
        static const int env_soa_w7 = getenv("GGML_MV_SOA_W7") ? atoi(getenv("GGML_MV_SOA_W7")) : 0;
        const bool use_soa_w7 = env_soa_w7 && ne11 == 7 && ne12 == 1 && ne13 == 1 && use_f16y && use_di &&
                                op->src[0]->type == GGML_TYPE_Q4_0 && ne00%64 == 0 &&
                                ggml_metal_mul_mat_soa_w4_rows(ne01);
        static const int env_soa_w5 = getenv("GGML_MV_SOA_W5") ? atoi(getenv("GGML_MV_SOA_W5")) : 0;
        static const int env_soa_w5_hp = getenv("GGML_MV_SOA_W5_HALF") ? atoi(getenv("GGML_MV_SOA_W5_HALF")) : 0;
        const bool use_soa_w5 = env_soa_w5 && ne11 == 5 && ne12 == 1 && ne13 == 1 && use_f16y && use_di &&
                                op->src[0]->type == GGML_TYPE_Q4_0 && ne00%64 == 0 &&
                                ggml_metal_mul_mat_soa_w4_rows(ne01);
        ...
        const bool use_soa_w6 = env_soa_w6 && ne11 == 6 && ne12 == 1 && ne13 == 1 && use_f16y && use_di &&
                                op->src[0]->type == GGML_TYPE_Q4_0 && ne00%64 == 0 &&
                                ggml_metal_mul_mat_soa_w4_rows(ne01);
