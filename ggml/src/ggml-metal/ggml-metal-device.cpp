...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mm_skinny(ggml_metal_library_t lib, const ggml_tensor * op, bool di) {
    ...
    const int ne13 = op->src[1]->ne[3];
    const int r2   = ne12 / op->src[0]->ne[2];
    const int r3   = ne13 / op->src[0]->ne[3];
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mm_skinny_soa(ggml_metal_library_t lib, const ggml_tensor * op) {
    char base[256];
    char name[256];
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mm_skinny_soa_n16(ggml_metal_library_t lib, const ggml_tensor * op) {
    char base[256];
    char name[256];
    ...
    const char * src0_name = op->src[0]->type == GGML_TYPE_Q4_0_SOA ? "q4_0" : ggml_type_name(op->src[0]->type);
    snprintf(base, 256, "kernel_mul_mm_skinny_%s_soa_n16_%s", src0_name, ggml_type_name(op->src[1]->type));
    snprintf(name, 256, "%s_ne12=%d_r2=%d_r3=%d", base, ne12, r2, r3);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_repack_q4_0_soa(ggml_metal_library_t lib) {
    const char * name = "kernel_repack_q4_0_soa";
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_q4_0_soa_w1(ggml_metal_library_t lib) {
    const char * name = "kernel_mul_mv_q4_0_soa_w1";
    auto res = ggml_metal_library_get_pipeline(lib, name);
    return res.pipeline ? res : ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
}

ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_q4_0_soa_w2(ggml_metal_library_t lib) {
    const char * name = "kernel_mul_mv_q4_0_soa_w2";
    auto res = ggml_metal_library_get_pipeline(lib, name);
    return res.pipeline ? res : ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
}
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_q4_0_soa_w4_k1(ggml_metal_library_t lib) {
    const char * name = "kernel_mul_mv_q4_0_soa_w4_k1";
    auto res = ggml_metal_library_get_pipeline(lib, name);
    return res.pipeline ? res : ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
}

ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_q4_0_soa_w4_r2(ggml_metal_library_t lib) {
    const char * name = "kernel_mul_mv_q4_0_soa_w4_r2";
    auto res = ggml_metal_library_get_pipeline(lib, name);
    return res.pipeline ? res : ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
}
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_q4_0_soa_w4_r4kp(ggml_metal_library_t lib, int variant) {
    const char * name = variant == 5 ? "kernel_mul_mv_q4_0_soa_w4_r4kp_v5" :
                        ...
                        variant == 2 ? "kernel_mul_mv_q4_0_soa_w4_r4kp_v2" :
                                       "kernel_mul_mv_q4_0_soa_w4_r4kp";
    auto res = ggml_metal_library_get_pipeline(lib, name);
    return res.pipeline ? res : ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
}
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_q4_0_soa_w5_qw(ggml_metal_library_t lib, int qw) {
    const char * name = qw == 4 ? "kernel_mul_mv_q4_0_soa_w5_r4hq4" :
                                  "kernel_mul_mv_q4_0_soa_w5_r4hq2";
    auto res = ggml_metal_library_get_pipeline(lib, name);
    return res.pipeline ? res : ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
}

ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_q4_0_soa_w5_skh(ggml_metal_library_t lib, int variant) {
    const char * name = variant == 3 ? "kernel_mul_mv_q4_0_soa_w5_skh_r8cs" :
                        variant == 2 ? "kernel_mul_mv_q4_0_soa_w5_skh_r8rs" :
                                       "kernel_mul_mv_q4_0_soa_w5_skh_r6";
    auto res = ggml_metal_library_get_pipeline(lib, name);
    return res.pipeline ? res : ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
}
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_ext(ggml_metal_library_t lib, const ggml_tensor * op, int nsg, int nxpsg, int r1ptg, int nr0, ggml_type tsrc1, bool di, int variant) {
    ...
    // stored q4_K tile scale: exact f32 d*sc by default (owner, 2026-09-06: "I want the exact version",
    // KLD-priced in weight-quant-kld.md); GGML_KQ_SOA_EXACT=0 reproduces upstream's half d/16 quotient
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mm(ggml_metal_library_t lib, const ggml_tensor * op, ggml_type tsrc1_override) {
    ...
    // stored SoA rows: the underlying type's tile with FC_mul_mm_soa (dequantize_soa_mm reads the row layout)
    const bool soa = ggml_metal_is_soa_type(tsrc0);
    const ggml_type tbase = ggml_metal_soa_base_type(tsrc0);
    ...
    static const bool kq_exact = !getenv("GGML_KQ_SOA_EXACT") || atoi(getenv("GGML_KQ_SOA_EXACT")) != 0;   // default on (see the ext builder)
    const bool kq_exact_here = kq_exact && tsrc0 == GGML_TYPE_Q4_K_SOA;
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext(
    ...
    if (op->src[1]->type != op->src[2]->type) {
        // mixed K/V types (TurboQuant asymmetric pairs) encode both types in the name
    ...
    if (!res.pipeline) {
        ...
        ggml_metal_cv_set_int32(cv, gqa_heads, FC_FLASH_ATTN_EXT + 24);
