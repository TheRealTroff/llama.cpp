...
struct test_gated_delta_net : public test_case {
    ...
    // ext semantics (ggml_gated_delta_net_ext): n_rep > 0 replays that many kept tokens per seq
    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
            ...
            if (strcmp(t->name, "g") == 0) {
            ...
            } else if (strcmp(t->name, "xrep") == 0) {
                std::vector<int32_t> rep(n_seqs);
                ...
                const int64_t H_v  = head_count * v_repeat;
                const int64_t g_ne0 = kda ? head_size : 1;
                const int64_t n_x  = t->ne[0];
                std::vector<float> data(ggml_nelements(t));
                std::default_random_engine rng(1234);
                std::uniform_real_distribution<float> u(-1.0f, 1.0f);
                for (int64_t r = 0; r < ggml_nelements(t)/n_x; ++r) {
                    float * x = data.data() + r*n_x;
                    for (int64_t h = 0; h < 2*head_count; ++h) {
                        float nrm = 0.0f;
                        for (int64_t i = 0; i < head_size; ++i) { x[h*head_size + i] = u(rng); nrm += x[h*head_size + i]*x[h*head_size + i]; }
                        nrm = 1.0f/std::sqrt(nrm + 1e-6f);
                        for (int64_t i = 0; i < head_size; ++i) { x[h*head_size + i] *= nrm; }
                    }
                    float * xv = x + 2*head_size*head_count;
                    for (int64_t i = 0; i < head_size*H_v; ++i) { xv[i] = -0.3f + 5.3f*(u(rng)*0.5f + 0.5f); }
                    float * xg = xv + head_size*H_v;
                    for (int64_t i = 0; i < g_ne0*H_v; ++i) { xg[i] = -20.0f + 20.0f*(u(rng)*0.5f + 0.5f) - 1e-4f; }
                    float * xb = xg + g_ne0*H_v;
                    for (int64_t i = 0; i < H_v; ++i) { xb[i] = u(rng)*0.5f + 0.5f; }
                }
                ggml_backend_tensor_set(t, data.data(), 0, data.size()*sizeof(float));
            } else {
                init_tensor_uniform(t);
...
struct test_flash_attn_ext : public test_case {
    ...
    test_flash_attn_ext(int64_t hsk = 128, int64_t hsv = 128, int64_t nh = 32, std::array<int64_t, 2> nr23 = {1, 1}, int64_t kv = 96, int64_t nb = 8,
                        ...
                        ggml_type type_K = GGML_TYPE_F16, ggml_type type_V = GGML_TYPE_F16, std::array<int32_t, 4> permute = {0, 1, 2, 3}, bool kv_view = true)
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // DFlash2 draft-model geometry.  Its 32 query heads share eight KV heads,
    // so these cover the GQA4 Turbo4 tile-reuse route at the production draft
    // depths.  KV=1088 is the standard 64-sink + 1024-window live cache.
    for (int nb : { 3, 4 }) {
        for (int kv : { 512, 1088 }) {
            test_cases.emplace_back(new test_flash_attn_ext(128, 128, 8, {4, 1}, kv, nb, true, false, 0, 0,
                                                            GGML_PREC_F32, GGML_TYPE_TURBO4_0, GGML_TYPE_TURBO4_0));
        }
    }
    test_cases.emplace_back(new test_flash_attn_ext(128, 128, 8, {4, 1}, 1088, 4, true, false, 0, 0,
                                                    GGML_PREC_F32, GGML_TYPE_TURBO4_0, GGML_TYPE_TURBO4_0, {0, 2, 1, 3}, false));
    ...
    // ext semantics: replay and keep (n_rep, n_keep)
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 4, 64, 1, 1, 1, false, false, 1, 0, 0));
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 4, 64, 4, 1, 1, false, false, 1, 0, 4));
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 4, 64, 4, 1, 1, false, false, 1, 0, 2));
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 4, 64, 3, 1, 1, false, false, 1, 2, 3));
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 4, 64, 1, 1, 1, false, false, 1, 3, 0));
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
 ...
 // High resolution output (large NPQ)
    ...
    for (int kv : { 1088, 8448, 102400 }) {
        test_cases.emplace_back(new test_flash_attn_ext(128, 128, 8, {4, 1}, kv, 4, true, false, 0, 0,
                                                        GGML_PREC_F32, GGML_TYPE_TURBO4_0, GGML_TYPE_TURBO4_0, {0, 2, 1, 3}, false));
    }
    ...
    // the 27B target's real GDN shape at the ubatch and verify widths: 16 k-heads, 48 v-heads (v_repeat 3),
    // plain and the pick's ext form (LLAMA_GDN_REPLAY: 2 slots, the last 4 tokens kept)
    for (int64_t nt : {512, 4}) {
        test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 16, 128, nt, 1, 3));
        test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 16, 128, nt, 1, 3, false, false, 1, 0, 4));
    }
